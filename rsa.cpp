...
void RSA::Produce_Keys()
{
	...
	_key.nkey = Produce_Nkey(prime1, prime2);

	bm::int1024_t orla = Produce_Oral(prime1, prime2);
	_key.ekey = Produce_Ekey(orla);

	_key.dkey = Produce_Dkey(_key.ekey, orla);
}
bm::int1024_t RSA::Produce_Nkey(bm::int1024_t prime1, bm::int1024_t prime2)
{
	//选择两个不相等的质数p，q，得 n = pq
	return prime1 * prime2;
}

bm::int1024_t RSA::Produce_Oral(bm::int1024_t prime1, bm::int1024_t prime2)
{
	//欧拉公式，f(n)
	//如果a是制数：f(n) = n - 1
...
bm::int1024_t RSA::Produce_Gcd(bm::int1024_t ekey, bm::int1024_t orla)
{
	...
	return RSA::Produce_Gcd(orla, ekey%orla);
}
bm::int1024_t RSA::Produce_Exgcd(bm::int1024_t ekey, bm::int1024_t orla, bm::int1024_t& x, bm::int1024_t& y)
{
	//扩展的欧几里得算法
	if (orla == 0)
	{
	...
	bm::int1024_t ret = Produce_Exgcd(orla, ekey%orla, x, y);
	bm::int1024_t tmpx = x;
	bm::int1024_t tmpy = y;
	x = tmpy;
...
bm::int1024_t RSA::Produce_Dkey(bm::int1024_t ekey, bm::int1024_t orla)
{
	...
	bm::int1024_t dkey = 1;
	bm::int1024_t Y = 3;
	bm::int1024_t tmp = Produce_Exgcd(ekey, orla, dkey, Y);
